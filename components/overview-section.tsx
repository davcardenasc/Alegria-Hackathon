      ...
      <div className="container mx-auto px-6 sm:px-8 lg:px-4">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((item, idx) => (
            <div
              key={idx}
              className={`relative bg-gradient-to-br ${item.color} border-2 ${item.border} rounded-xl p-6 hover:scale-105 hover:shadow-[0_0_30px_rgba(74,94,231,0.2)] transition-all duration-300 overflow-hidden`}
            >
              ...
              <div className="relative z-10">
                ...
                <h3 className="text-xl font-bold text-[#F7F9FF] mb-3">{item.title}</h3>
                <ul className="space-y-2">
                  {item.lines.map((line, i) => (
                    <li key={i} className="text-[#BFC9DB] text-sm leading-relaxed">
                      {line}
                    </li>
                  ))}
                </ul>
              </div>
              <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent -skew-x-12 translate-x-[-200%] hover:translate-x-[200%] transition-transform duration-1000" />
            </div>
          ))}
        </div>
